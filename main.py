...
print("Startup:", "ELEVENLABS_API_KEY set?", os.getenv("ELEVENLABS_API_KEY") is not None, "VOICE_ID:", os.getenv("ELEVENLABS_VOICE_ID"))
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS call_metadata (
...
def get_history(sid):
    def _(): 
        ...
        if not row:
            c.execute("INSERT INTO conversations(call_sid,messages,reprompt_count) VALUES(?,?,0)",(sid,json.dumps([])))
            conn.commit()
        conn.close()
        return msgs
...
def save_booking(sid,vt,pn,dd):
    def _():
        conn=sqlite3.connect(DB_PATH); c=conn.cursor()
        c.execute("""INSERT INTO bookings(call_sid,vaccine_type,patient_name,desired_date)
                     VALUES(?,?,?,?);""",(sid,vt,pn,dd))
        conn.commit(); conn.close()
    retry_sqlite(_)
...
def classify_intent(text):
    t=text.lower()
    if any(k in t for k in ["vaccine","shot","vaccination"]): return "VACCINE"
...
def generate_and_play_tts(text,sid,suffix="resp"):
    ...
    try:
        ...
        hd={"xi-api-key":ELEVENLABS_API_KEY,"Content-Type":"application/json"}
        payload={"text":text,"model_id":"eleven_multilingual_v2",
                 "voice_settings":{"stability":0.5,"similarity_boost":0.5}}
        r=requests.post(url,json=payload,headers=hd,timeout=10)
        if r.status_code!=200 or not r.content:
            print("TTS error",r.status_code,r.text); raise Exception()
        with open(fp,"wb") as f: f.write(r.content)
        vr=VoiceResponse()
        gr=vr.gather(input="speech",action=f"{BASE_URL}/process-recording",
...
async def incoming_call(req: Request):
    ...
    sid=f.get("CallSid")
    md=(sid,f.get("From"),f.get("FromCity"),f.get("FromState"),f.get("FromZip"),f.get("FromCountry"))
    conn=sqlite3.connect(DB_PATH); c=conn.cursor()
...
async def process_recording(req: Request):
    ...
    if not sid: return Response(status_code=400)
    hist=get_history(sid); reps=get_reprompt_count(sid)

    # Escalation
    for kw in ("urgent","emergency","immediately","asap"):
        if kw in us.lower():
            esc="Emergency detected—redirecting you to a pharmacist now. Please hold."
            log_call_turn(sid,len(hist)//2+1,us,esc,"EMERGENCY_OBSERVED")
            hist.append({"role":"assistant","content":esc}); save_history(sid,hist)
    ...
    if not us.strip():
        ...
        if reps<3:
            increment_reprompt_count(sid); log_call_turn(sid,len(hist)//2,None,None,"Silence reprompt")
        else:
            log_call_turn(sid,len(hist)//2,None,None,"Silence hangup"); vr.hangup()
    ...
    if conf<0.5:
        ...
        log_call_turn(sid,len(hist)//2,us,None,f"Low conf ({conf})")
        tw=str(vr); print("📤 process-recording TwiML:",tw)
        return Response(content=tw,media_type="application/xml")

    # Add user
    hist.append({"role":"user","content":us.strip()})
    reset_reprompt_count(sid)

    # Vaccine flow initial
    ...
    if intent=="VACCINE":
        ...
        reply="We’re open Monday–Friday 9 AM–6 PM, and Saturday 10 AM–4 PM."
    elif intent=="NEAREST":
        # GEOCODE via Google Maps
        postal=us.strip().replace(" ","")
        try:
            geo=requests.get(
                f"https://maps.googleapis.com/maps/api/geocode/json"
                f"?address={postal},Canada&key={GOOGLE_MAPS_API_KEY}",timeout=5
            ).json()
            loc=geo["results"][0]["geometry"]["location"]
            places=requests.get(
                f"https://maps.googleapis.com/maps/api/place/nearbysearch/json"
                f"?location={loc['lat']},{loc['lng']}&radius=5000&type=pharmacy&key={GOOGLE_MAPS_API_KEY}",timeout=5
            ).json()["results"][:3]
            names= [f"{p['name']} at {p['vicinity']}" for p in places]
            reply="Here are the closest pharmacies: "+ "; ".join(names)+"."
        except:
            reply="Sorry, I couldn’t find nearby pharmacies. Could you try another postal code?"
    else:
        # default fallback GPT
        few=[{"role":"system","content":"You are concise; keep replies under 300 characters."}]
        resp=openai_client.chat.completions.create(model="gpt-3.5-turbo",
                                                   messages=few+hist,temperature=0.2)
        reply=resp.choices[0].message.content.strip()

    hist.append({"role":"assistant","content":reply})
    save_history(sid,hist)
    log_call_turn(sid,len(hist)//2,us,reply,None)

    vr=generate_and_play_tts(reply,sid,str(int(time.time())))
    tw=str(vr); print("📤 process-recording TwiML:",tw)
    return Response(content=tw,media_type="application/xml")
...
async def get_call_logs(limit: int=100):
    conn=sqlite3.connect(DB_PATH); c=conn.cursor()
    c.execute("""SELECT id,call_sid,turn_number,user_text,assistant_reply,
