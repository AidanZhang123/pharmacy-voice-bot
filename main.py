...
print("Startup:", "ELEVENLABS_API_KEY?", os.getenv("ELEVENLABS_API_KEY") is not None, "VOICE_ID:", os.getenv("ELEVENLABS_VOICE_ID"))
...
def log_call_turn(sid,turn,ut,ar,err):
    def _():
        conn=sqlite3.connect(DB_PATH); c=conn.cursor()
        c.execute("INSERT INTO call_logs(call_sid,turn_number,user_text,assistant_reply,error_message) VALUES(?,?,?,?,?);",(sid,turn,ut,ar,err))
        conn.commit(); conn.close()
    retry_sqlite(_)
...
def save_booking(sid,vt,pn,dd):
    def _():
        conn=sqlite3.connect(DB_PATH); c=conn.cursor()
        c.execute("INSERT INTO bookings(call_sid,vaccine_type,patient_name,desired_date) VALUES(?,?,?,?);",(sid,vt,pn,dd))
        conn.commit(); conn.close()
    retry_sqlite(_)
...
def classify_intent(t): 
    l=t.lower()
    if any(k in l for k in ["vaccine","shot"]): return "VACCINE"
    if any(k in l for k in ["refill","renew","prescription"]): return "REFILL"
    if any(k in l for k in ["hour","open","close","time"]): return "HOURS"
    if "pharmacy" in l: return "NEAREST"
    return "GENERAL"

# ─── ElevenLabs TTS Helper ───────────────────────────────────────────────────────
def generate_and_play_tts(text,sid,suffix="resp"):
    fn=f"tts_{sid}_{suffix}_{int(time.time())}.mp3"; fp=os.path.join("static",fn)
    try:
        url=f"https://api.elevenlabs.io/v1/text-to-speech/{VOICE_ID}"
        hd={"xi-api-key":ELEVENLABS_API_KEY,"Content-Type":"application/json"}
        pl={"text":text,"model_id":"eleven_multilingual_v2","voice_settings":{"stability":0.5,"similarity_boost":0.5}}
...
async def incoming_call(req:Request):
    f=await req.form(); print("📥 incoming-call:",dict(f))
    sid=f.get("CallSid")
    md=(sid,f.get("From"),f.get("FromCity"),f.get("FromState"),f.get("FromZip"),f.get("FromCountry"))
    conn=sqlite3.connect(DB_PATH); c=conn.cursor()
    c.execute("INSERT OR REPLACE INTO call_metadata(call_sid,from_number,from_city,from_state,from_zip,from_country) VALUES(?,?,?,?,?,?);",md)
    conn.commit(); conn.close()
...
async def process_recording(req:Request):
    f=await req.form(); print("📥 process-recording:",dict(f))
